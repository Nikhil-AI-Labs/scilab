...
// ---------- TIME DOMAIN PLOTS ----------
fig1 = figure("position", [100,100,1200,800]);  // Bigger window
subplot(3,2,1);
plot(t,y_msg,'LineWidth',2);
title('Message Signal');
xlabel('Time (s)'); ylabel('Amplitude');
gca().font_size = 3;

subplot(3,2,2);
plot(t,y_carrier,'LineWidth',2);
title('Carrier Signal');
xlabel('Time (s)'); ylabel('Amplitude');
gca().font_size = 3;

subplot(3,2,3);
plot(t,y_dsb_sc,'LineWidth',2);
title('DSB-SC Signal (Suppressed Carrier)');
xlabel('Time (s)'); ylabel('Amplitude');
gca().font_size = 3;

subplot(3,2,4);
plot(t,y_dsb_fc,'LineWidth',2);
title('DSB-FC Signal (AM with Carrier)');
xlabel('Time (s)'); ylabel('Amplitude');
gca().font_size = 3;

subplot(3,2,5);
plot(t,y_dem_sc,'LineWidth',2);
title('Demodulated Message (from DSB-SC)');
xlabel('Time (s)'); ylabel('Amplitude');
gca().font_size = 3;

subplot(3,2,6);
plot(t,y_dem_fc,'LineWidth',2);
title('Demodulated Message (from DSB-FC)');
xlabel('Time (s)'); ylabel('Amplitude');
gca().font_size = 3;

// ---------- FREQUENCY DOMAIN PLOTS ----------
fig2 = figure("position", [200,200,1200,800]);  // Bigger window
subplot(3,2,1);
plot(f_axis,MSG,'LineWidth',2);
title('Spectrum of Message Signal');
xlabel('Frequency (Hz)'); ylabel('Magnitude');
gca().font_size = 3;

subplot(3,2,2);
plot(f_axis,CARR,'LineWidth',2);
title('Spectrum of Carrier Signal');
xlabel('Frequency (Hz)'); ylabel('Magnitude');
gca().font_size = 3;

subplot(3,2,3);
plot(f_axis,DSBSC,'LineWidth',2);
title('Spectrum of DSB-SC Signal');
xlabel('Frequency (Hz)'); ylabel('Magnitude');
gca().font_size = 3;

subplot(3,2,4);
plot(f_axis,DSBFC,'LineWidth',2);
title('Spectrum of DSB-FC Signal');
xlabel('Frequency (Hz)'); ylabel('Magnitude');
gca().font_size = 3;

subplot(3,2,5);
plot(f_axis,DEM_SC,'LineWidth',2);
title('Spectrum of Demodulated Message (DSB-SC)');
xlabel('Frequency (Hz)'); ylabel('Magnitude');
gca().font_size = 3;

subplot(3,2,6);
plot(f_axis,DEM_FC,'LineWidth',2);
